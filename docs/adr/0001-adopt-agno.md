...
The project's core requirement is to translate natural language user requests into executable SQL queries. This involves complex interactions with a Large Language Model (LLM), including managing prompts, defining agent roles, providing tools for execution, and handling the model's responses. Building this infrastructure from scratch using raw API calls would be time-consuming, error-prone, and would require reinventing common patterns for agentic systems. We need a framework that abstracts this complexity and provides a structured way to build and manage AI agents.

## Decision

We will adopt the **`agno` library** as the foundational framework for building our AI agent system. We will leverage its core components:

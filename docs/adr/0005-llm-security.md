...
We will implement a **layered security model** to harden the application against common LLM attack vectors. This includes:
...
3. **Output Validation:** We will maintain the existing code-based check within the `execute_sql_query` tool, which validates that the final query is a `SELECT` statement, providing a final layer of defense.
